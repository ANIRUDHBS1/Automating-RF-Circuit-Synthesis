...
def gradient(ms, mg, R2, R1, Ib1, Ib2): #Needs update here
      #Needs edit
      global loss_dict
      lr = 1
      dms = 0
      ...
      if loss_dict['loss_S11'] != 0:  
      	    dmg = dmg + lr * (20/(mg-5)) 
      grads = [dms, dmg, dR2, dR1, dIb1, dIb2]
      return grads
...
def update(): #Needs update here
    ...
    ms = parameter_dict['ms']
    mg = parameter_dict['mg']
    R2 = parameter_dict['R2']
    R1 = parameter_dict['R1']
    Ib1 = parameter_dict['Ib1']
    Ib2 = parameter_dict['Ib2']
    grads = gradient(ms, mg, R2, R1, Ib1, Ib2)
    parameter_dict['ms'] = parameter_dict['ms'] - grads[0]  
    parameter_dict['mg'] = parameter_dict['mg'] - grads[1]  
    parameter_dict['R2'] = parameter_dict['R2'] - grads[2]  
    parameter_dict['R1'] = parameter_dict['R1'] - grads[3]  
    parameter_dict['Ib1'] = parameter_dict['Ib1'] - grads[4]   
    parameter_dict['Ib2'] = parameter_dict['Ib2'] - grads[5]
...
def DCop(file_name): #Update to add more details
    ...
    #DCopdict["idsMg"] = df['M1:ids'].values[0]
    DCopdict["gmMg"] = df['M1:gm'].values[0]
    DCopdict["gdsMg"] = df['M1:gds'].values[0]
    DCopdict["VDsatMg"] = df['M1:vdsat'].values[0]
    DCopdict["vthMg"] = df['M1:vth'].values[0]
...
def optimization():
    ...
    DCvals = DCop(dcout)
    print(f"Parameters are {parameter_dict}")
    Idd = DCvals['Idd']
    print(f"The DC operating point is : \n Common Gate DCOP - gm : {DCvals['gmMg']} S, gds : {DCvals['gdsMg']} S, VDSAT : {DCvals['VDsatMg']}V \n Common Source DCOp - gm : {DCvals['gm']} S, gds : {DCvals['gds']} S, VDSAT : {DCvals['VDsat']}V. \n The current drawn from VDD is Idc = {Idd}A \n The power consumed is {0.9*Idd} W")
    ...
    if loss_dict['loss'] < 1.70 :
    	...
    	linesx = copy.copy(lines)
    	lineshbx = copy.copy(lineshb)
    	writelines(linesx, netlistnew)
    	writelines(lineshbx, netlistnewhb)
    update()
    updatelines(lines)
    updatelines(lineshb)
...
#-------------------------------Optimization Function without Printing-----------------------------------
...
#-------------------------------Function to get sensitivity (Right sided sensitivity) -----------------------------------
def sensitivity(file_name):
    lines = getfile(file_name)
    ...
    for parameter in parameter_dict:
         if parameter == 'ms' or parameter == 'mg' or parameter == 'R1' or parameter == 'R2' or parameter == 'Ib1' or parameter == 'Ib2':
...
#-------------------------------Function to get process being run (eg. TOP_FF) -----------------------------------
...
#-------------------------------Function to get Temperature Sensitivity -----------------------------------
def tempsensitivity(file_name):
    lines = getfile(file_name)
    run_spectre(file_name)
    loss_i =  optimizationTemp()
    iloss = copy.copy(loss_i)
...
#-------------------------------Function to update the lines for PVT Variations -----------------------------------
...
#-------------------------------Function to get the loss at different temperatures -----------------------------------
loss_dict_temp = {}
def temp_analysis(netlist1, netlist2):
...
#-------------------------------Function to loss at different processes -----------------------------------
loss_dict_process = {}
def process_analysis(netlist1, netlist2):
...
#-------------------------------Function to get loss at various supply voltages -----------------------------------
loss_dict_voltage = {}
def voltage_analysis(netlist1, netlist2):
...
def main():
    ...
    plt.ylabel("Loss")
    plt.title("Loss as a function of iteration number")
    plt.show()
    for netlistidx in range(noofnetlistsaved):
    	...
    	getparameters(lines)
    	temp_analysis(netlist, netlisthb)
    	print(' ')
    	process_analysis(netlist, netlisthb)
    	print(' ')
    	voltage_analysis(netlist, netlisthb)
    	print(' ')
    	dloss = sensitivity(netlist)
    	dtloss = tempsensitivity(netlist)
